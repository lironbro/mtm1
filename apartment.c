...
static bool split_check_and_price(Apartment apartment,int* price1, int* price2,
		bool splitByRow, int index);

static bool split_create_new(Apartment apartment,Apartment* first, Apartment* second,
		bool splitByRow, int index, int price1, int price2);
...
{
	if(apartment == NULL || outArea == NULL || apartment->squares == NULL)
		return APARTMENT_NULL_ARG;
                                ...
                                int index, Apartment* first, Apartment* second)
{
    ...
    if(splitByRow)
    {
    	if(index < 0 || index > apartment->length)
    	    return APARTMENT_OUT_OF_BOUNDS;
    	if(!split_check_and_price(apartment, &price1, &price2, true, index))
    		return APARTMENT_BAD_SPLIT;
    	*first = apartmentCreate(apartment->squares, index, apartment->width,
    			price1);
    	*second = apartmentCreate(&apartment->squares[index+1],
    			apartment->length-index-1, apartment->width, price2);
    }
    else
    {
    	if(index < 0 || index > apartment->width)
    		return APARTMENT_OUT_OF_BOUNDS;
    	if(!split_check_and_price(apartment, &price1, &price2, false, index))
    	    		return APARTMENT_BAD_SPLIT;
    	if(!split_create_new(apartment, first, second, splitByRow,
    			index, price1, price2))
...
{
	for(int i=0; i<apartment->length; i++)
	    if(apartment->squares[(splitByRow)?index:i][(splitByRow)?i:index]
	    		== EMPTY)
	    	return false;
	if(splitByRow) {
		*price1 = apartment->price*(index+1)/(apartment->length);
		*price2 = apartment->price*(apartment->length-index)/(apartment->length);
	}
	else {
		*price1 = (apartment->price)*(index+1)/(apartment->width);
		*price2 = (apartment->price)*(apartment->width-index)/(apartment->width);
	}
	return true;
}

static bool split_create_new(Apartment apartment,Apartment* first,
		Apartment* second, bool splitByRow, int index, int price1, int price2)
{
	if(splitByRow) {
	...
	*first = apartmentCreate(apartment->squares, apartment->length, index,
			price1);
	*second = apartmentCreate(squares, apartment->length,
			apartment->width-index-1, price2);
	for(int i=0; i<apartment->length; i++) {
		free(squares[i]);
	}
	free(squares);
                                    ...
                                    SquareType value)
{
	if(apartment == NULL || apartment->squares == NULL)
		return APARTMENT_NULL_ARG;
    if(row >= apartment->length || col >= apartment->width
    		|| row < 0 || col < 0)
        return APARTMENT_OUT_OF_BOUNDS;
